...
DListIterator<T> DList<T>::remove(DListIterator<T> &iter)
{
	...
	{
		DListIterator<T> previousNodeIterator(this, iter.currentNode->previous);
		if (iter.currentNode == tail)
		{
			tail = previousNodeIterator.currentNode;
			tail->next = nullptr;
		}
		previousNodeIterator.currentNode->next = iter.currentNode->next;
		if (iter.currentNode->next != nullptr)
		{
			iter.currentNode->next->previous = previousNodeIterator.currentNode;
		}
		delete iter.currentNode;
		count--;
		return DListIterator<T>(this, previousNodeIterator.currentNode->next);
